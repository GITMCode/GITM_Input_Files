...
subroutine aurora(iBlock)
  ...
  real :: eflx_pa_ergs, av_pa_kev, avee_pa, E0_pa, eflux_pa, Q0_pa, a_pa
  ...
  do i=1,nLats
     do j=1,nLons
        ...
        eflx_pa_ergs = ElectronPAEnergyFlux(j,i)
        av_pa_kev    = ElectronPAAverageEnergy(j,i) * AveEFactor
        ...
        if (eflx_ergs > 0.1) then
           ...
           avee_pa = av_pa_kev * 1000.0
           eflux_pa = eflx_pa_ergs * 6.242e11 
           ...
           do n=1,ED_N_Energies
              ...
              if (IsKappaAurora) then
                 ...
                 if(UsePulsatingAurora) then
                    ED_flux(n) = ED_flux(n) + a_pa * ed_energies(n) * exp(-ed_energies(n)/E0_pa)
                 endif
