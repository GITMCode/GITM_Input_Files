...
subroutine set_inputs
  ...
  do while (.not. IsDone)
     ...
     if (cLine(1:1) == "#") then
        ...
        case ("#USEPULSATINGAURORA")
           call read_in_logical(UsePulsatingAurora, iError)
           call read_in_real(PAAveE, iError)
           call read_in_real(PAEFlux, iError)
           call read_in_real(PAPeriod, iError)
           call read_in_real(PAMltCenter, iError)
           call read_in_real(PALatCenter, iError)
           call read_in_real(PAMltHalfWidth, iError)
           call	read_in_real(PALatHalfwidth, iError)
           if (iError /= 0) then
              write(*,*) 'Incorrect format for #USEPULSATINGAURORA'
              write(*,*) 'Use this for specifying the pulsating aurora.'
              write(*,*) ''
              write(*,*) '#USEPULSATINGAURORA'
              write(*,*) 'UsePulsatingAurora        (logical)'
              write(*,*) 'PAAveE                    (real)'
              write(*,*) 'PAEFlux                   (real)'
              write(*,*) 'PAPeriod                  (real)'
              write(*,*) 'PAMltCenter               (real)'
              write(*,*) 'PALatCenter               (real)'
              write(*,*) 'PAMltHalfWidth            (real)'
              write(*,*) 'PALatHalfWidth            (real)'
              IsDone = .true.
           endif
