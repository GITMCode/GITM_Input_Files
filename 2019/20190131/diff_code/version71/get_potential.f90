...
subroutine get_potential(iBlock)
  ...
  real, dimension(-1:nLons+2, -1:nLats+2) :: EPAFlux
  ...
  if (floor((tSimulation-dt)/DtAurora) /= &
    ...
    if (UsePulsatingAurora) then
       ...
       if (maxval(lats) > 50) then
          ...
          EPAFlux = PAEFlux * sin(mod(tSimulation,PAPeriod)*2.0*PI) * &
               exp(-abs(lats - PALatCenter)/PALatHalfWidth) *  &
               exp(-abs(mlts - PAMltCenter)/PAMltHalfWidth)

          do iLat=-1,nLats+2
             do iLon=-1,nLons+2
                if (EPAFlux(iLon,iLat) > 0.1) then
                   ElectronPAEnergyFlux(iLon, iLat) = EPAFlux(iLon, iLat)
                   ElectronPAAverageEnergy(iLon, iLat) = PAAveE
                endif
             enddo
          enddo
       endif
